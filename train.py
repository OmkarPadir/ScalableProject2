...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        Rsymbols_file.close()
        replaced_captcha_symbols=captcha_symbols


        for i in range(len(fromSymbol)):
            replaced_captcha_symbols= replaced_captcha_symbols.replace(fromSymbol[i],toSymbol[i])

        # print(replaced_captcha_symbols)
        # new code ends

        training_data = ImageSequence(args.train_dataset, args.batch_size, args.length, replaced_captcha_symbols, args.width, args.height)
        validation_data = ImageSequence(args.validate_dataset, args.batch_size, args.length, replaced_captcha_symbols, args.width, args.height)
